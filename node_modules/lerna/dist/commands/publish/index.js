...
function collectUncommitted({ cwd, log: log49 = import_npmlog.default }) {
  log49.silly("collect-uncommitted", "git status --porcelain (async)");
...
async function detectProjects(packageConfigs) {
  ...
  let projectFileMap;
  if (import_devkit4.createProjectFileMapUsingProjectGraph) {
    projectFileMap = await (0, import_devkit4.createProjectFileMapUsingProjectGraph)(_projectGraph);
  } else {
...
function getCycles(dependencies) {
  ...
  function dfs(next, path19) {
    ...
    for (const dep of dependencies[next] || []) {
      if (path19.includes(dep.target)) {
        const cycle = path19.slice(path19.indexOf(dep.target));
        cycles.push(cycle);
      } else if (!visited.has(dep.target)) {
        dfs(dep.target, path19);
      }
    }
    path19.pop();
  }
...
var import_fs_extra5, import_libnpmpublish, import_npm_package_arg5, import_npmlog26, import_path13, import_pify2, readJSON, readJSONAsync;
...
var import_npm_packlist, import_npmlog29, import_path16, import_tar2;
...
var import_node_fetch, import_npmlog34, import_path17, GitLabClient;
...
function getCurrentSHA(opts) {
  import_npmlog37.default.silly("getCurrentSHA", "");
  const sha = childProcess10.execSync("git", ["rev-parse", "HEAD"], opts);
  import_npmlog37.default.verbose("getCurrentSHA", sha);
  return sha;
}
...
var import_npm_package_arg7, import_npmlog38, childProcess11;
...
async function getProjectsWithTaggedPackages(projectNodes, projectFileMap, execOpts) {
  import_npmlog40.default.silly("getTaggedPackages", "");
...
var import_npmlog40, childProcess12;
...
async function getProjectsWithUnpublishedPackages(projectNodes, opts) {
  import_npmlog41.default.silly("getProjectsWithUnpublishedPackages", "");
  const mapper = (node) => {
    ...
    return import_pacote.default.packument(pkg.name, opts).then(
      ...
      () => {
        import_npmlog41.default.warn("", "Unable to determine published version, assuming %j unpublished.", pkg.name);
        return node;
      }
    );
  };
...
var import_npmlog41, import_p_map4, import_pacote;
...
function getCurrentBranch(opts) {
  import_npmlog42.default.silly("getCurrentBranch");
  const branch = childProcess13.execSync("git", ["rev-parse", "--abbrev-ref", "HEAD"], opts);
  import_npmlog42.default.verbose("currentBranch", branch);
  return branch;
}
...
function resolvePrettier() {
  if (!resolvedPrettier) {
    try {
      ...
      const prettierPath = import_path20.default.join(import_devkit6.workspaceRoot, "node_modules", "prettier");
...
function maybeFormatFile(filePath) {
  ...
  if (resolvedPrettier.getFileInfo.sync(fullFilePath, { ignorePath }).ignored) {
    import_npmlog43.default.silly("version", `Skipped applying prettier to ignored file: ${filePath}`);
    return;
  }
  try {
    ...
    import_npmlog43.default.silly("version", `Successfully applied prettier to updated file: ${filePath}`);
  } catch {
    import_npmlog43.default.silly("version", `Failed to apply prettier to updated file: ${filePath}`);
  }
}
function gitAdd(changedFiles, gitOpts, execOpts) {
  ...
  import_npmlog43.default.silly("gitAdd", files);
  return childProcess14.exec("git", ["add", "--", ...files], execOpts);
}
var import_devkit6, import_fs4, import_npmlog43, import_path20, import_slash2, childProcess14, resolvedPrettier;
...
function gitCommit(message, { amend, commitHooks, signGitCommit, signoffGitCommit }, opts) {
  import_npmlog44.default.silly("gitCommit", message);
  const args = ["commit"];
  ...
  import_npmlog44.default.verbose("git", args);
  return childProcess15.exec("git", args, opts);
}
var import_npmlog44, import_os3, childProcess15;
...
function gitPush(remote, branch, opts) {
  import_npmlog45.default.silly("gitPush", remote, branch);
  return childProcess16.exec("git", ["push", "--follow-tags", "--no-verify", "--atomic", remote, branch], opts).catch((error) => {
    if (/atomic/.test(error.stderr) || process.env.GIT_REDIRECT_STDERR === "2>&1" && /atomic/.test(error.stdout)) {
      import_npmlog45.default.warn("gitPush", error.stderr);
      import_npmlog45.default.info("gitPush", "--atomic failed, attempting non-atomic push");
      return childProcess16.exec("git", ["push", "--follow-tags", "--no-verify", remote, branch], opts);
    }
    throw error;
  });
}
var import_npmlog45, childProcess16;
...
function gitTag(tag, { forceGitTag, signGitTag }, opts, command = "git tag %s -m %s") {
  import_npmlog46.default.silly("gitTag", tag, command);
  ...
  import_npmlog46.default.verbose(cmd, interpolatedArgs);
  return childProcess17.exec(cmd, interpolatedArgs, opts);
}
var import_npmlog46, childProcess17;
...
function isAnythingCommitted(opts) {
  import_npmlog47.default.silly("isAnythingCommitted", "");
  const anyCommits = childProcess18.execSync("git", ["rev-list", "--count", "--all", "--max-count=1"], opts);
  import_npmlog47.default.verbose("isAnythingCommitted", anyCommits);
  return Boolean(parseInt(anyCommits, 10));
}
var import_npmlog47, childProcess18;
...
var import_npmlog48, childProcess19;
...
function remoteBranchExists(gitRemote, branch, opts) {
  import_npmlog49.default.silly("remoteBranchExists", "");
  const remoteBranch = `${gitRemote}/${branch}`;
  try {
...
var import_npmlog49, childProcess20;
...
async function updateLockfileVersion(pkg) {
  ...
  const obj = await (0, import_load_json_file3.default)(lockfilePath).catch(() => {
    import_npmlog50.default.verbose("version", `${pkg.name} has no lockfile. Skipping lockfile update.`);
  });
  if (!obj) {
    return;
  }
...
var import_devkit7, import_load_json_file3, import_npmlog50, import_path21;
    ...
    var PublishCommand = class extends Command {
      ...
      async execute() {
        ...
        if (this.options.summaryFile !== void 0) {
          ...
          output(jsonObject);
          try {
            import_fs5.default.writeFileSync(filePath, JSON.stringify(jsonObject));
      ...
      async detectFromGit() {
        ...
        if (!taggedPackageNames.length) {
          ...
          const newVersion = import_semver8.default.maxSatisfying(tags, "*");
      ...
      async copyAssets(pkg) {
        ...
        for (const file of filesToCopy) {
          ...
          } else if ((0, import_fs5.existsSync)(file.from)) {
            this.logger.verbose(
              "publish",
